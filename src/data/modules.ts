...
const STATIC: Record<string, StaticConfig> = {
  ...
  'error-scenarios': {
    ...
    subFeatures: [
      ...
      { name: 'Error tracking — Amplitude Page_Error event (status, message, domain)', done: false },
      { name: 'Error tracking — backend TrackPageHitsApi on error', done: false },
      ...
      { name: 'Auth — OTP send failed generic toast', done: false },
      { name: 'Auth — OTP: "Please enter the complete 6-digit code"', done: false },
      ...
      { name: 'Auth — OTP resend cooldown: "Please wait 60 seconds"', done: false },
      { name: 'Auth — OTP: 429 rate limit → wait 5 minutes message', done: true },
      { name: 'Auth — OTP: 403 verification blocked → contact support', done: false },
      ...
      { name: 'Upload — Meta upload failed: file too large / bad format / corrupted message', done: false },
      { name: 'Upload — 401 authentication failed for file upload', done: false },
      { name: 'Upload — 403 permission denied for file upload', done: false },
      { name: 'Upload — audio conversion error toast', done: false },

      // ── 8. Inline Operation Errors ────────────────────────────────────────
      { name: 'Message send failed — error icon on bubble + retry button', done: true },
      { name: 'Message send failed — failed messages dropdown in top bar', done: false },
      { name: 'Failed messages dropdown — message, reason, timestamp, retry', done: false },
      { name: 'Template error on send — routes to Message Flows tab', done: false },
      { name: '"Template not approved" send error', done: false },
      { name: '"Template deleted or never existed" send error', done: false },
      { name: '"Template not submitted for this channel" error', done: false },
      { name: 'Ticket duplicate failed toast', done: false },
      { name: '"Cannot delete the last view" CRM error', done: false },
      { name: 'Webhook request failed toast', done: false },

      // ── 9. Permission / Access Errors ─────────────────────────────────────
      { name: 'Scope-locked feature — disabled UI with tooltip', done: false },
      { name: '"You do not have permission to create team links" toast', done: false },
      { name: '"You do not have permission to invite team member" toast', done: false },
      { name: 'Plan-gated feature — upgrade prompt / paywall UI', done: false },
      { name: 'Feature locked page — FeatureLockedPage with upgrade CTA', done: false },
      { name: '"Contact Support" disabled action tooltip', done: false },

      // ── 10. Channel / WABA Errors ─────────────────────────────────────────
      { name: 'QR code expired state — Refresh QR button', done: false },
      { name: 'QR code loading state — spinner overlay', done: false },
      { name: 'QR code generation error — unexpected error message', done: false },
      { name: 'Channel disconnected / broken state display', done: false },
      { name: 'WABA quality rating — GREEN / YELLOW / RED with tooltip', done: false },
      { name: 'WABA submission status — pending / rejected / approved badge', done: false },
      { name: 'WABA ISV terms — rejected status display', done: false },
      { name: 'Coexist upgrade — "not supported for this country code" error', done: false },
      { name: 'Coexist upgrade — "Meta does not support this region" error', done: false },
      { name: 'Channel setup — "Button feature not supported" limitation', done: false },
      { name: 'Cannot login to service error (OAuth integrations)', done: false },
      { name: 'Zapier connection blocked by privacy settings error', done: false },

      // ── 11. Payment / Billing Errors ──────────────────────────────────────
      { name: 'Insufficient credits — 402 intercept + top-up CTA', done: false },
      { name: '"Oops! No payment setup" compose warning', done: false },
      { name: '"Oops! No product setup" compose warning', done: false },
      { name: '"Please log in to continue with checkout" error', done: false },
      { name: '"Plan pricing not found. Please contact support" error', done: false },
      { name: '"Failed to generate payment link" error', done: false },
      { name: '"Enterprise plans require custom setup" error', done: false },
      { name: 'Invalid coupon code error', done: false },
      { name: '"Cannot use your own referral code" coupon error', done: false },

      // ── 12. Flow Builder Errors ───────────────────────────────────────────
      { name: 'Flow save errors modal — lists all node validation errors', done: false },
      { name: 'Flow save errors modal — "Jump to node" button per error', done: false },
      { name: 'Flow save errors modal — singular vs plural error count title', done: false },
      { name: 'Node validation — "Starting action must have message text or attachment"', done: false },
      { name: 'Node validation — "Action must have text, buttons or list"', done: false },
      { name: 'Template approval — rejected templates list with count', done: false },
      { name: 'Template approval — rejection reason from Meta with remediation advice', done: false },
      { name: 'Template approval — pending / not submitted / approved status badges', done: false },

      // ── 13. CRM / Inbox Content Errors ───────────────────────────────────
      { name: 'Ticket AI analysis error — "unexpected error, please try again"', done: false },
      { name: 'Message transcription error — video corrupted / no audio', done: false },
      { name: 'Unsupported message type — "This message type is not supported"', done: false },
      { name: 'Metric loading error — "trouble loading metric, refresh or contact support"', done: false },
