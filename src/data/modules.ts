...
const STATIC: Record<string, StaticConfig> = {
  ...
  dashboard: {
    ...
    subFeatures: [
      ...
      { name: 'Per-widget filter', done: false },
    ...
    notes: 'Core widget grid infrastructure is solid: react-grid-layout drag-drop, 5 widget types (line, bar, pie, snapshot, table), dashboard create/delete, AddWidgetDialog, TimePeriodSelector, NewDashboardModal, TeamAnalyticsView. Major gaps vs old app: no per-widget breakdown or filter, no aggregate grouping selector, no period-vs-period comparison rendering, no CSV/PNG export, no dashboard sharing or rename, no performance tabs, no channel/tag filter bar.',
  },
