...
const STATIC: Record<string, StaticConfig> = {
  ...
  inbox: {
    ...
    subFeatures: [
      ...
      { name: 'Mark as read / unread toggle from chat list row', done: true },
      { name: 'Date range filter in chat list header', done: true },
      { name: 'Manage custom fields via 3-dot menu on chat list row', done: true },
      ...
      { name: 'Export chat history (PDF)', done: true },
      ...
      { name: 'Profile tab — group participants panel hidden for non-group chats (bug fix)', done: true },
