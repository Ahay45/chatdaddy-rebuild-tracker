...
const STATIC: Record<string, StaticConfig> = {
  ...
  inbox: {
    ...
    subFeatures: [
      ...
      { name: 'Mark as read / unread toggle from chat list row', done: false },
      { name: 'Date range filter in chat list header', done: false },
      { name: 'Manage custom fields via 3-dot menu on chat list row', done: false },
      ...
      { name: 'Export chat history (PDF)', done: false },
      ...
      { name: 'Profile tab — group participants panel hidden for non-group chats (bug fix)', done: false },
  ...
  'flow-builder': {
    ...
    subFeatures: [
      ...
      { name: 'ReactFlow canvas with infinite pan and zoom', done: true },
      { name: 'Drag-and-drop nodes onto canvas', done: true },
      { name: 'Multi-select nodes (shift-click / drag-box)', done: true },
      { name: 'Helper lines / snap-to-align when dragging nodes', done: true },
      { name: 'Minimap in canvas corner', done: true },
      { name: 'Fit-view / zoom-to-fit button', done: true },
      { name: 'Keyboard shortcuts: copy, paste, delete, undo, redo', done: true },
      { name: 'Copy-paste selected nodes (with edges, preserving layout)', done: true },
      { name: 'Undo / redo history stack', done: true },
      { name: 'Auto-layout (reset / re-arrange nodes)', done: true },
      { name: 'Canvas background (dots / grid pattern)', done: true },
      { name: 'Node drag-handle (dedicated grip area per node)', done: true },

      // ── Node Types ───────────────────────────────────────────────────────
      // Trigger node
      { name: 'Trigger node — keyword match (exact / contains / regex)', done: true },
      { name: 'Trigger node — event trigger (new contact, order, webhook, etc.)', done: true },
      { name: 'Trigger node — trigger frequency (always / once per contact / throttle interval)', done: true },
      { name: 'Trigger node — audience filter (include / exclude contact lists)', done: true },
      { name: 'Trigger node — time window (only fire between hours / days)', done: true },
      { name: 'Trigger node — timezone setting per trigger', done: true },
      { name: 'Trigger node — channel selector (which WhatsApp number to listen on)', done: true },
      { name: 'Trigger node — multiple keywords per trigger', done: true },

      // Message node
      { name: 'Message node — plain text message', done: true },
      { name: 'Message node — image attachment', done: true },
      { name: 'Message node — video attachment', done: true },
      { name: 'Message node — audio attachment', done: true },
      { name: 'Message node — document / file attachment', done: true },
      { name: 'Message node — interactive button list (up to 10 buttons)', done: true },
      { name: 'Message node — interactive list message (sections + rows)', done: true },
      { name: 'Message node — location message', done: true },
      { name: 'Message node — WhatsApp template (HSM) message', done: true },
      { name: 'Message node — template variable mapping', done: true },
      { name: 'Message node — dynamic variable injection ({{contact.name}}, custom fields)', done: true },
      { name: 'Message node — assign chat to agent / team after send', done: true },
      { name: 'Message node — add / remove tags after send', done: true },
      { name: 'Message node — update contact field after send', done: true },
      { name: 'Message node — send delay before this message (seconds)', done: true },
      { name: 'Message node — rich text editor with formatting toolbar', done: true },

      // Condition node
      { name: 'Condition node — AND / OR logic groups', done: true },
      { name: 'Condition node — contact field operators (equals, contains, starts with, regex, empty, not empty)', done: true },
      { name: 'Condition node — numeric operators (greater than, less than, between)', done: true },
      { name: 'Condition node — date / time operators (before, after, within last N days)', done: true },
      { name: 'Condition node — tag presence condition', done: true },
      { name: 'Condition node — custom attribute condition', done: true },
      { name: 'Condition node — True / False output edges', done: true },
      { name: 'Condition node — nested condition groups', done: true },
      ...
      { name: 'Input node — free-text reply capture', done: true },
      { name: 'Input node — number / phone / email validation on reply', done: true },
      { name: 'Input node — save reply to contact custom field', done: true },
      { name: 'Input node — save reply to conversation attribute', done: true },
      { name: 'Input node — invalid input branch (retry or fallback)', done: true },
      { name: 'Input node — timeout branch (no reply within N minutes)', done: true },
      { name: 'Input node — max retries before fallback', done: true },
      { name: 'Input node — button-choice reply capture', done: true },

      // Webhook / URL node
      { name: 'Webhook node — HTTP GET / POST / PUT / DELETE', done: true },
      { name: 'Webhook node — custom headers and auth', done: true },
      { name: 'Webhook node — JSON body with variable interpolation', done: true },
      { name: 'Webhook node — response mapping (store fields to contact attributes)', done: true },
      { name: 'Webhook node — success / failure output edges', done: true },
      { name: 'Webhook node — test request from builder', done: true },

      // Email node
      { name: 'Email node — subject / body template', done: true },
      { name: 'Email node — dynamic variable injection in email', done: true },
      { name: 'Email node — sender name and reply-to config', done: true },
      { name: 'Email node — HTML vs plain-text toggle', done: true },

      // Form embed node
      { name: 'Form node — link to internal ChatDaddy form', done: true },
      { name: 'Form node — send form link as message', done: true },
      { name: 'Form node — listen for form submission to continue flow', done: true },

      // Bot target node
      { name: 'Bot target node — jump to another flow / bot', done: true },
      { name: 'Bot target node — pass context variables to target flow', done: true },
      { name: 'Bot target node — return-to-parent after target completes', done: true },

      // App integration nodes
      { name: 'Integration node — Shopify: create / update order', done: true },
      { name: 'Integration node — Shopify: look up order status', done: true },
      { name: 'Integration node — Google Sheets: append / update row', done: true },
      { name: 'Integration node — Google Calendar: create event', done: true },
      { name: 'Integration node — Calendly: get availability / create booking', done: true },
      { name: 'Integration node — Stripe: create payment link', done: true },
      { name: 'Integration node — OpenAI / AI response node', done: true },
      { name: 'Integration node — custom API integration (generic)', done: true },

      // Shape / annotation nodes
      { name: 'Note / comment node (sticky note on canvas)', done: true },
      { name: 'Group / frame node (visual grouping of nodes)', done: true },

      // ── Edges & Connections ──────────────────────────────────────────────
      { name: 'Animated edges (dashed animation showing flow direction)', done: true },
      { name: 'Edge labels', done: true },
      { name: 'Edge delete on click', done: true },
      { name: 'Reconnect edge by dragging endpoint', done: true },
      { name: 'Self-loop edges (node connects back to itself)', done: true },
      { name: 'Multiple output edges from one node handle', done: true },

      // ── Node Editor Sidebar ──────────────────────────────────────────────
      { name: 'Sidebar opens on node click (slide-in panel)', done: true },
      { name: 'Sidebar — node name / label editor', done: true },
      { name: 'Sidebar — node-specific property form (dynamic per node type)', done: true },
      { name: 'Sidebar — live preview of message as WhatsApp bubble', done: true },
      { name: 'Sidebar — media upload (image / video / audio / document)', done: true },
      { name: 'Sidebar — emoji picker in message editor', done: true },
      { name: 'Sidebar — variable picker (insert {{variable}} from dropdown)', done: true },
      { name: 'Sidebar — close / collapse sidebar button', done: true },

      // ── Node Toolbar (context menu on node) ─────────────────────────────
      { name: 'Node toolbar — duplicate node', done: true },
      { name: 'Node toolbar — delete node', done: true },
      { name: 'Node toolbar — add note / comment to node', done: true },
      { name: 'Node toolbar — enable / disable node (bypass in flow)', done: true },

      // ── Toolbar & Top Bar ────────────────────────────────────────────────
      { name: 'Flow name editable in top bar', done: true },
      { name: 'Save button (manual save)', done: true },
      { name: 'Publish / activate flow toggle', done: true },
      { name: 'Unsaved changes indicator (dirty state badge)', done: true },
      { name: 'Conflict detection — warn when another user edited the flow', done: true },
      { name: 'Back button (return to flow list)', done: true },
      { name: 'Test flow button (send test to self)', done: true },
      { name: 'Flow settings button (opens meta panel)', done: true },

      // ── Flow Management (list page) ──────────────────────────────────────
      { name: 'Flow list page with search', done: true },
      { name: 'Flow list — folder / category organization', done: true },
      { name: 'Flow list — create new flow', done: true },
      { name: 'Flow list — duplicate existing flow', done: true },
      { name: 'Flow list — delete flow (with confirmation)', done: true },
      { name: 'Flow list — rename flow inline', done: true },
      { name: 'Flow list — active / inactive status badge', done: true },
      { name: 'Flow list — last modified timestamp', done: true },
      { name: 'Flow list — flow analytics summary (sent / delivered counts)', done: true },
      { name: 'Flow import from JSON file', done: true },
      { name: 'Flow export to JSON file', done: true },
      { name: 'Flow share via template link', done: true },

      // ── Template Library ─────────────────────────────────────────────────
      { name: 'Template gallery / browse page', done: true },
      { name: 'Template preview before import', done: true },
      { name: 'Import template into flow builder', done: true },
      { name: 'Export flow as reusable template', done: true },
      { name: 'Community / marketplace templates', done: true },

      // ── Flow Analytics ───────────────────────────────────────────────────
      { name: 'Flow analytics — total contacts entered', done: true },
      { name: 'Flow analytics — contacts at each node (funnel view)', done: true },
      { name: 'Flow analytics — node-level sent / delivered / read rates', done: true },
      { name: 'Flow analytics — button click-through rates per button', done: true },
      { name: 'Flow analytics — drop-off points / exit nodes', done: true },
      { name: 'Flow analytics — date-range filter', done: true },
      { name: 'Flow analytics — export analytics CSV', done: true },
      { name: 'Flow analytics — heatmap overlay on canvas (color nodes by volume)', done: true },

      // ── AI Flow Builder ──────────────────────────────────────────────────
      { name: 'AI builder — prompt input ("describe your flow in plain language")', done: true },
      { name: 'AI builder — generate flow from description', done: true },
      { name: 'AI builder — review / edit generated nodes before applying', done: true },
      { name: 'AI builder — iterate / refine with follow-up prompts', done: true },

      // ── Bot / Flow Settings ──────────────────────────────────────────────
      { name: 'Flow settings — description / notes field', done: true },
      { name: 'Flow settings — default timezone', done: true },
      { name: 'Flow settings — stop on agent reply (pause bot when human responds)', done: true },
      { name: 'Flow settings — opt-out keyword (stop word exits flow)', done: true },
      { name: 'Flow settings — error / fallback message', done: true },
      { name: 'Flow settings — assign label / tag on flow entry', done: true },

      // ── Audience / Contact Targeting ─────────────────────────────────────
      { name: 'Audience filter on trigger — include contact lists', done: true },
      { name: 'Audience filter on trigger — exclude contact lists', done: true },
      { name: 'Audience filter — filter by tag', done: true },
      { name: 'Audience filter — filter by custom attribute', done: true },
      { name: 'Audience filter — estimated audience size preview', done: true },

      // ── Condition Logic Operators ─────────────────────────────────────────
      { name: 'Condition operator — equals / not equals', done: true },
      { name: 'Condition operator — contains / not contains', done: true },
      { name: 'Condition operator — starts with / ends with', done: true },
      { name: 'Condition operator — regex match', done: true },
      { name: 'Condition operator — is empty / is not empty', done: true },
      { name: 'Condition operator — greater than / less than / between', done: true },
      { name: 'Condition operator — before date / after date / within N days', done: true },

      // ── Message Content Types (inside Message node) ───────────────────────
      { name: 'Message content — text with emoji support', done: true },
      { name: 'Message content — image with caption', done: true },
      { name: 'Message content — video with caption', done: true },
      { name: 'Message content — audio (voice note style)', done: true },
      { name: 'Message content — PDF / document with filename', done: true },
      { name: 'Message content — location pin (lat/lon + label)', done: true },
      { name: 'Message content — contact card (vCard)', done: true },
      { name: 'Message content — catalog message (product)', done: true },
      { name: 'Message content — multi-product message', done: true },
      { name: 'Message content — CTA URL button', done: true },
      { name: 'Message content — quick reply buttons', done: true },

      // ── Delay Types (inside Delay node) ──────────────────────────────────
      { name: 'Delay type — wait N seconds / minutes / hours', done: true },
      { name: 'Delay type — wait until specific date-time (absolute)', done: true },
      { name: 'Delay type — wait until next occurrence of weekday', done: true },
      { name: 'Delay type — wait until business hours window opens', done: true },

      // ── Demo / Preview Mode ───────────────────────────────────────────────
      { name: 'Demo mode — simulate flow execution step by step', done: true },
      { name: 'Demo mode — highlight active node as simulation progresses', done: true },
      { name: 'Demo mode — show simulated message output per node', done: true },
      { name: 'Test send — send flow to a real phone number for live test', done: true },

      // ── Data Models / Variable System ─────────────────────────────────────
      { name: 'Variable system — built-in contact variables (name, phone, email)', done: true },
      { name: 'Variable system — custom contact fields as variables', done: true },
      { name: 'Variable system — flow-scoped variables (set / get within flow)', done: true },
      { name: 'Variable system — webhook response fields as variables', done: true },
      { name: 'Variable system — input node capture → variable', done: true },
      { name: 'Variable system — date/time helper variables (today, now)', done: true },

      // ── Mobile / Responsive ───────────────────────────────────────────────
      { name: 'Mobile — canvas is scrollable and zoomable on touch', done: true },
      { name: 'Mobile — sidebar stacks below canvas on small screens', done: true },
      { name: 'Mobile — touch-drag to move nodes', done: true },
    ],
  ...
  'status-page': {
    ...
    subFeatures: [
      ...
      { name: 'Standalone public URL (status.chatdaddy.com)', done: true },
      { name: 'Overall system status banner (Operational / Degraded Performance / Partial Outage / Major Outage)', done: true },
      { name: 'Status banner color coding (green / yellow / orange / red)', done: true },
      { name: 'Auto-refresh page every 60 seconds without full reload', done: true },
      { name: 'Last updated timestamp displayed', done: true },
      { name: 'Favicon updates to reflect current status (green dot / red dot)', done: true },

      // ── Component Status Grid ─────────────────────────────────────────────
      { name: 'Per-component status list (each service as a row)', done: true },
      { name: 'Component: Dashboard (web app)', done: true },
      { name: 'Component: API (REST endpoints)', done: true },
      { name: 'Component: WhatsApp Messaging (message delivery)', done: true },
      { name: 'Component: WhatsApp Channels (channel connections)', done: true },
      { name: 'Component: Webhook Delivery', done: true },
      { name: 'Component: Automation / Flow Builder execution', done: true },
      { name: 'Component: Broadcast / Campaign delivery', done: true },
      { name: 'Component: AI Chatbot / AI features', done: true },
      { name: 'Component: CRM / Contacts sync', done: true },
      { name: 'Component: Billing & Payments', done: true },
      { name: 'Component: Media Upload / File Storage', done: true },
      { name: 'Component: Real-time / WebSocket connections', done: true },
      { name: 'Component status icon per row (Operational / Degraded / Outage / Maintenance)', done: true },
      { name: 'Component group / category collapsing (e.g. "Core Platform", "Messaging", "Integrations")', done: true },
      { name: 'Sub-component expand (e.g. API → REST v1, REST v2, Webhooks)', done: true },

      // ── Uptime History Bar (90-day chart) ─────────────────────────────────
      { name: 'Uptime history bar — 90-day daily squares per component', done: true },
      { name: 'Color-coded daily squares (green = 100%, yellow = degraded, red = outage, grey = no data)', done: true },
      { name: 'Hover tooltip on each day square showing date + uptime %', done: true },
      { name: 'Uptime percentage label for last 90 days (e.g. "99.87% uptime")', done: true },
      { name: 'Toggle: 30-day / 60-day / 90-day uptime view', done: true },

      // ── Active Incidents Banner ────────────────────────────────────────────
      { name: 'Active incident banner at top (shown only when incident is open)', done: true },
      { name: 'Incident severity badge (Investigating / Identified / Monitoring / Resolved)', done: true },
      { name: 'Incident title and latest update in banner', done: true },
      { name: 'Link from banner to full incident detail', done: true },
      { name: 'Multiple simultaneous incidents stacked in banner', done: true },

      // ── Incident List (public) ────────────────────────────────────────────
      { name: 'Incident history section below components', done: true },
      { name: 'Incidents grouped by date (Today, This week, Past incidents)', done: true },
      { name: 'Incident row — title, affected components, status badge, duration', done: true },
      { name: 'Incident detail page (full timeline of updates)', done: true },
      { name: 'Incident update timeline — each update with timestamp and message', done: true },
      { name: 'Incident resolution time displayed', done: true },
      { name: 'Affected components listed per incident', done: true },
      { name: 'Load more / paginate older incidents', done: true },

      // ── Scheduled Maintenance ─────────────────────────────────────────────
      { name: 'Upcoming scheduled maintenance section', done: true },
      { name: 'Maintenance card — title, affected components, start/end time, timezone', done: true },
      { name: 'Maintenance status: Scheduled / In Progress / Completed', done: true },
      { name: 'Maintenance countdown ("Starts in 2 hours")', done: true },
      { name: 'Past maintenance history section', done: true },
      ...
      { name: 'Admin — create new incident (title, severity, affected components, initial message)', done: true },
      { name: 'Admin — update incident status (Investigating → Identified → Monitoring → Resolved)', done: true },
      { name: 'Admin — add update message to existing incident', done: true },
      { name: 'Admin — resolve / close incident', done: true },
      { name: 'Admin — schedule maintenance (title, start, end, affected components, message)', done: true },
  ...
  'analytics-ux-checklist': {
    ...
    subFeatures: [
      // 1. Adoption Tracking
      { name: 'feature_viewed event', done: true },
      { name: 'feature_clicked event', done: true },
      { name: 'Unique users using feature tracked', done: true },
      { name: 'First-time usage tracked', done: true },

      // 2. Interaction Tracking
      { name: 'Click events tracked', done: true },
      { name: 'Dead clicks detected', done: true },
      { name: 'Rage clicks detected', done: true },
      { name: 'Hover tracking', done: true },
      { name: 'Scroll depth tracking', done: true },

      // 3. Engagement Metrics
      { name: 'Time spent per session tracked', done: true },
      { name: 'Time to complete task tracked', done: true },
      { name: 'Idle vs active time tracked', done: true },
      { name: 'Number of steps taken tracked', done: true },

      // 4. Funnel Tracking (CRITICAL)
      { name: 'Funnel steps defined per module', done: true },
      { name: 'Step-by-step conversion tracked', done: true },
      { name: 'Drop-off points identified', done: true },
      { name: 'Completion rate tracked', done: true },

      // 5. Errors & Friction
      { name: 'Error messages tracked', done: true },
      { name: 'Validation failures tracked', done: true },
      { name: 'API failures tracked', done: true },
      { name: 'Loading time tracked', done: true },
      { name: 'User exits tracked', done: true },

      // 6. UX Quality Signals
      { name: 'Rage click rate surfaced per module', done: true },
      { name: 'Dead click rate surfaced per module', done: true },
      { name: 'Backtracking behavior tracked', done: true },
      { name: 'Repeated attempts tracked', done: true },

      // 7. Derived Metrics
      { name: 'Task success rate calculated', done: true },
      { name: 'Avg completion time calculated', done: true },
      { name: 'Drop-off rate calculated', done: true },
      { name: 'UX difficulty score calculated', done: true },

      // Visual Indicators (per-module status display)
      { name: 'Per-module tracking status badge (✅ / ⚠️ / ❌)', done: true },
      { name: 'Checklist completion % per module', done: true },
      { name: 'Critical missing items highlighted (red)', done: true },

      // Smart Insights Layer
      { name: 'Auto-generated insight: high drop-off detection', done: true },
      { name: 'Auto-generated insight: clicks without completion', done: true },
      { name: 'Auto-generated insight: high rage click rate warning', done: true },

      // Data Source Integration
      { name: 'PostHog / Mixpanel / Amplitude integration ready', done: true },
      { name: 'Event naming convention enforced (module_feature_action)', done: true },
      { name: 'Custom event tracking system hookup', done: true },

      // UI / Checklist Shell
      { name: 'Collapsible category panels UI', done: true },
      { name: 'Progress bars per checklist category', done: true },
      { name: 'Color-coded health status (green / yellow / red)', done: true },
      { name: 'Per-module analytics readiness dashboard view', done: true },
    ],
    notes: 'Complete. ux-tracker.ts: rage/dead click detection, scroll depth, time tracking, funnel steps, Amplitude/Mixpanel/PostHog adapters, event naming enforcement. UXChecklistPanel: 44-item checklist in 6 categories, collapsible panels, progress bars, color-coded health, derived metrics (success rate, difficulty score, drop-off rate), smart insights. Accessible as tab in Admin page.',
  },
  ...
  'error-scenarios': {
    ...
    subFeatures: [
      ...
      { name: 'Validation — "Please Select an Active Channel"', done: true },
      { name: 'Validation — "Please fill in all WhatsApp numbers"', done: true },
      { name: 'Validation — form question incomplete (flow/form builder)', done: true },
      { name: 'Validation — WABA ISV terms required fields (name, email, address, description, country)', done: true },
      { name: 'Validation — billing amount too low', done: true },
      { name: 'Validation — "Please select a plan or power-ups to continue"', done: true },
      { name: 'Validation — chatbot missing name/description/industry before publish', done: true },
      { name: 'Validation — duplicate flow name warning', done: true },
      { name: 'Validation — team name / company name required', done: true },
      { name: 'Validation — "This user already exists"', done: true },
