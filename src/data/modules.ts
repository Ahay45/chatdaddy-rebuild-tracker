...
const STATIC: Record<string, StaticConfig> = {
  ...
  dashboard: {
    ...
    subFeatures: [
      ...
      { name: 'Add / edit dashboard widgets', done: true },
      { name: 'Dashboard layout manager (drag-and-drop widgets)', done: true },
      { name: 'Dashboard sharing', done: false },
      { name: 'New dashboard creation modal', done: true },
      ...
      { name: 'Snapshot / comparison chart', done: true },
      { name: 'Table / data grid chart', done: true },
      { name: 'Chart legends and tooltips', done: true },
      { name: 'Metric comparison view (period vs period)', done: true },
      { name: 'Team analytics view', done: true },
    ],
    notes: 'Full analytics rebuild complete. WidgetGrid (react-grid-layout drag-drop, 5 widget types), DashboardSelector (scrollable tabs), TimePeriodSelector, AddWidgetDialog, NewDashboardModal, TeamAnalyticsView (agent performance table), SnapshotWidget, LineChartWidget, BarChartWidget, PieChartWidget, TableWidget all built. Remaining: dashboard sharing.',
  },
